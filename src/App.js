...
function App() {
    ...
    async function fetchData() {
      try {
        const [cartResponse, favoritesResponse] = await Promise.all([
          axios.get('https://63091d01f8a20183f76ec73c.mockapi.io/cart'),
          axios.get('https://63091d01f8a20183f76ec73c.mockapi.io/favorites'),
        ]);
  ...
  const onAddToFavorite = async (obj) => {
    try {
      ...
      if (findFavoriteItem) {
        ...
        await axios.delete(
          `https://63091d01f8a20183f76ec73c.mockapi.io/favorites/${findFavoriteItem.id}`,
        );
      } else {
        setFavorites((prev) => [...prev, obj]);
        const { data } = await axios.post(
          'https://63091d01f8a20183f76ec73c.mockapi.io/favorites',
          obj,
        );
        setFavorites((prev) =>
          prev.map((item) => {
  ...
  return (
    <AppContext.Provider
      value={{ cartItems, favorites, onAddToCart, setCartItems, onAddToFavorite, isItemFavorite }}>
      <div className="App">
        <HeaderLine />
        <Header onChange={searchValueChange} />
        ...
        <Routes>
          ...
          <Route
            path="/cart"
            element={<Cart cartItems={cartItems} onRemoveFromCard={onRemoveFromCard} />}
          />

          <Route
            path="/favorites"
            element={<Favorites favorites={favorites} filteredFavorites={filteredFavorites} />}
          />
        </Routes>
      </div>
    </AppContext.Provider>
